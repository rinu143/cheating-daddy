...
function getConfigDir() {
    ...
    if (platform === 'win32') {
        // Windows: %APPDATA%\nvidia-premier-config
        configDir = path.join(os.homedir(), 'AppData', 'Roaming', 'nvidia-premier-config');
    } else if (platform === 'darwin') {
        // macOS: ~/Library/Application Support/nvidia-premier-config
        configDir = path.join(os.homedir(), 'Library', 'Application Support', 'nvidia-premier-config');
    } else {
        // Linux and others: ~/.config/nvidia-premier-config
        configDir = path.join(os.homedir(), '.config', 'nvidia-premier-config');
    }
